...
データ分析としては「収集 → 蓄積 → 加工 → 分析 → 可視化」 / すけージュル管理 / 通知 ..etc など必要事項がたくさんありますが、`蓄積後の整形〜可視化` にフォーカスした構成をやってみます。
...
| 種別     | OSS          | 役割                                 |
| ------ | ------------ | ---------------------------------- |
| データベース | **DuckDB**   | オブジェクトストレージ上の CSV を直接読み込み、分析クエリを実行 |
| モデリング  | **dbt**      | SQL モデル化とカラム統一、ファクト/ディメンション生成      |
| 可視化    | **Superset** | DuckDB 上のデータからダッシュボード作成            |

### 2.2. 利用するさくらのクラウド
利用するは次の 3つです。

- サーバー（VM）: DuckDB / dbt を同梱
- サーバー（VM）: Superset 
シンプルモードで構築したサーバーの例です。
![](/images/19_sakura_cloud_data_kiban/1_make_sakura_server.png)

- オブジェクトストレージ: raw データ（CSV）の置き場（APIキーを取得）
...
そのまま集計すると誤って、税抜と税込金額を合計したりなどによる意図しない誤集計が発生する可能性があります。
...
こちらはデータでは作成したデータなのですが、不定期で上がったり下がったりあるのは面白いですね。
...
今回作ってみながら、もっとコード管理や自動化などを考えたりで楽しくも時間をかけてしまったので、少しずつ改良しながらより良い基盤を作れたらなと感じております。
